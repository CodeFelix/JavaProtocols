...
public class Simplex_rcv extends Base_Protocol implements Callbacks {
    ...
    public void from_physical_layer(long time, Frame frame) {
        ...
        if (frame.kind() == Frame.DATA_FRAME) {                                                     // Check the frame kind -> DATA 
            ...
            else{                                                                                   //  Se a trama recebida não estiver na sequencia certa
                Frame ack_frame_received = Frame.new_Ack_Frame(prev_seq(frame_expected), null);     //  Cria um novo objecto do tipo ACK com o ACK anterior 
                sim.to_physical_layer(ack_frame_received); 
            }
